...
Vector3 Curve::evalBezier(double t) {
    ...
    if (nbPoint()>1) {
        ...
        for(int k = 0; k< castel.size(); k++)
            for(int i = 0; i<castel.size() -k -1; i++)
...
Vector3 Curve::evalCubicVelocity(double t) {
    ...
    res=t*t*3*(-point(0)+3*point(1)-3*point(2)+point(3))+
            t*2*(3*point(0)-6*point(1)+3*point(2))+
            (-3*point(0)+3*point(1));
...
Vector3 Curve::evalCubicAcceleration(double t) {
    ...
    Vector3 res;
    res=t*6*(-point(0)+3*point(1)-3*point(2)+point(3))+
