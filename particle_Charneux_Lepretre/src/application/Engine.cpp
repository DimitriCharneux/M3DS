...
<<<<<<< HEAD
        ...
        double res = d+(position.x()*normal.x() + (position.y()-p->radius())*normal.y() + position.z()*normal.z());
        double k = (plane->point() - p->position()).dot(normal)/(normal.dot(normal));
        Vector3 H = p->position()+k*normal;
        ...
        if(res <= 0){
            ...
            //Exo2Q4
            /*posCorrection.add(H - p->position());
...
<<<<<<< HEAD
          ...
          double k = computeImpulse(p1,p2,eloignement, restitution);
          if((p2->position() - p1->position()).length() < p2->radius() + p1->radius()){
              /*posCorrectionP1 = (1+restitution)*p1->mass() / (p1->mass()+p2->mass()) * recouvrement * eloignement + p1->position();
