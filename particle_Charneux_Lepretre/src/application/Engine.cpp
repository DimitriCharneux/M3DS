...
void Engine::euler(double dt) {
    for(unsigned int i=0; i<_particleList->size(); i++) {
        ...
        if (p->alive()) {
            /* Affecter la nouvelle vitesse et la nouvelle position de p avec p->velocity(??), et p->position(??).
             */
...
void Engine::computeForce() {
    for(unsigned int i=0; i<_particleList->size(); i++) {
        Particle *p=(*_particleList)[i];
        if (p->alive()) {
            /* Appliquer les forces subies */
            p->addForce(Vector3(0,-1 * p->mass() * 9.81,0));
    ...
    for(unsigned int j=0; j<_planeList.size(); j++) {
        ...
        for(unsigned int i=0; i<_particleList->size(); i++) {
            ...
            if (p->alive()) {
                ...
                /* Détecter collision et corriger vitesse/position */
