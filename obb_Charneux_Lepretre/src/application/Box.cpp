...
bool Box::detectCollision(Box *b1,Box *b2,CollisionInfo *collision) {
    ...
    // - déterminez la distance minimale dist_min de recouvrement entre les 4 axes axis[i] qui sont non séparateurs
    //(Attention : minimale en valeur absolue !, mais dist_min est négative s'il y a recouvrement)
    // - vous devez affecter correctement axe_min (l'axe correspondant à dist_min) qui est un des axis[i] *mais* en
    //              tenant compte du sens de séparation de
    ...
    */
    ...
    //premier cas, necessaire de separer pour remplir dist et axe min
    distance(b1,b2,axis[0],&dist,&direction);
    dist_min = dist;
